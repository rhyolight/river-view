...
Lockmaster.prototype.assertEnvironmentVariablesExist = function() {
    var me = this;
    _.each(me.rivers, function(river) {
        if (river.config.env) {
            _.each(river.config.env, function(key) {
                if (process.env[key] === undefined) {
                    var message = 'Cannot start river "' + river.name +
                        '" because expected environment variable "' + key +
                        '" does not exist.';
                    river.config.disabled = true;
...
Lockmaster.prototype.initializeRivers = function(callback) {
    var me = this,
        redisClient = me.redisClient,
        initializers = {},
        timeFetchers = {},
        initializationFailures = {};

    _.each(me.rivers, function(r) {
...
Lockmaster.prototype.start = function start(startCallback) {
    ...
    me.initializeRivers(function(initializationFailures, sourceUrls) {
        ...
        me.assertEnvironmentVariablesExist();
