 ...
 * @param opts.dataClient {RedisClient} Redis client.
...
River.prototype.saveTemporalData = function(streamId, timestamp, data) {
    this.dataClient.writeRiverTemporalData(this.name, streamId, timestamp, data, this.expires, function(error) {
...
River.prototype.saveMetaData = function(streamId, data) {
    this.dataClient.writeRiverMetaData(this.name, streamId, data, this.expires, function(error) {
        // TODO: handle this error properly.
        if (error) throw error;
    });
};
...
River.prototype.getMetaData = function(streamId, callback) {
    this.dataClient.getRiverMetaData(this.name, streamId, function(err, meta) {
...
River.prototype.getTemporalData = function(streamId, query, callback) {
    ...
    this.dataClient.getRiverData(this.name, streamId, myQuery, function(err, data) {
        if (err) return callback(err);
        if (field) {
...
River.prototype.getKeys = function(options, callback) {
    this.dataClient.getKeys(this.name, options, callback);
};
...
River.prototype.getDataCount = function(streamId, callback) {
    this.dataClient.getDataCount(this.name, streamId, callback);
};
...
River.prototype.getFirstAndLastUpdatedTimes = function(streamId, callback) {
    ...
    me.dataClient.getLatestTimestampForRiverStream(riverName, streamId, function(err, timestamp) {
        var latest;
        if (err) return callback(err);
        latest = moment.unix(timestamp).tz(timezone);

        me.dataClient.getEarliestTimestampForRiverStream(riverName, streamId, function(err, timestamp) {
            var earliest;
            if (err) return callback(err);
